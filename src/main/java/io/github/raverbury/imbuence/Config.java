...
public class Config {
    ...
    public static final ForgeConfigSpec.DoubleValue THERMOSHOCK_BONUS_DAMAGE_PER_LEVEL;
    public static final ForgeConfigSpec.DoubleValue THERMOHEX_PERCENT_MAX_HEALTH_DOT_BASE;
    public static final ForgeConfigSpec.DoubleValue THERMOHEX_PERCENT_MAX_HEALTH_DOT_PER_LEVEL;
    ...
    static {
        ...
        OVERCAP_MAX_COST = CONFIG_BUILDER.comment("""
        ...
        FORTRESS_MAX_HEALTH_RATIO_GROWTH = CONFIG_BUILDER.comment("""
                        Fortress's max health ratio per level in decimal form.""")
                .defineInRange("fortress_max_health_ratio_per_level", 0.01, 0,
                        0.1);

        FORTRESS_SCALES_WORSE_AS_MAX_HEALTH_INCREASE = CONFIG_BUILDER.comment(
                        """
        ...
        COMMANDER_BONUS_DAMAGE_PER_PET = CONFIG_BUILDER.comment("""
        ...
        COMMANDER_MAX_BONUS_DAMAGE = CONFIG_BUILDER.comment("""
                        The max bonus damage provided by Commander,
                        to discourage having too many pets causing entity lag.""")
                .defineInRange("commander_max_bonus_damage", 5.0, 0.0,
                        Double.MAX_VALUE);

        CRIME_PUNISHMENT_BONUS_DAMAGE_PER_LEVEL = CONFIG_BUILDER.comment("""
        ...
        PET_QUERY_RANGE = CONFIG_BUILDER.comment(
                        "The range to check for nearby pets.")
                .defineInRange("pet_query_range", 12.0, 1.0, 64.0);

        FORBIDDEN_SYZYGY_APPLIES_GLOWING = CONFIG_BUILDER.comment(
                        "Controls whether Forbiden Syzygy applies Glowing to" + " entities hit by arrows.")
                .define("forbidden_syzygy_applies_glowing", true);

        PUZZLE_4_APPLIES_GLOWING = CONFIG_BUILDER.comment(
                        "Controls whether Puzzle 4 effect applies Glowing to" + " attacker.")
                .define("puzzle_4_applies_glowing", true);

        PUZZLE_6_DAMAGE_REDUCTION = CONFIG_BUILDER.comment("""
        ...
        PUZZLE_6_DAMAGE_REDUCTION_CAP = CONFIG_BUILDER.comment("""
        ...
        ROCKET_SPECIALIST_BONUS_DAMAGE_PER_LEVEL = CONFIG_BUILDER.comment("""
                ...
                .defineInRange("rocket_specialist_bonus_damage_per_level",
                        0.25d, 0d, Double.MAX_VALUE);

        ROCKET_SPECIALIST_BONUS_RADIUS_PER_LEVEL = CONFIG_BUILDER.comment("""
        ...
        SANCTA_POTENTIA_BONUS_FLAT_DAMAGE = CONFIG_BUILDER.comment(
                        "Sancta Potentia causes arrows to deal this much " + "extra flat damage.")
                .defineInRange("sancta_potentia_bonus_flat_damage", 6d, 0d,
                        Double.MAX_VALUE);

        MARE_POTENTIA_BONUS_PERCENT_DAMAGE = CONFIG_BUILDER.comment("""
                ...
                .defineInRange("mare_potentia_bonus_percent_damage", 0.7d, 0d,
                        Double.MAX_VALUE);

        PUZZLE_5_EFFECT_DURATION_EXTENSION = CONFIG_BUILDER.comment("""
        ...
        PUZZLE_5_EFFECT_DURATION_EXTENSION_POTION = CONFIG_BUILDER.comment("""
                        Puzzle 5's neutral and positive effects' duration
                        is further multiplied by this value when drinking potions,
                        in decimal form.""")
                .defineInRange("puzzle_5_effect_duration_extension_potion",
                        1.33d, 1d, Double.MAX_VALUE);

        PUZZLE_5_EFFECT_DURATION_REDUCTION = CONFIG_BUILDER.comment("""
                        Puzzle 5's negative effects' duration
                        is multiplied by this value, in decimal form.""")
                .defineInRange("puzzle_5_effect_duration_reduction", 0.67d, 0d,
                        1d);

        THERMOSHOCK_BONUS_DAMAGE_PER_LEVEL = CONFIG_BUILDER.comment("""
                        Thermoshock does this much damage per level.""")
                .defineInRange("thermoshock_bonus_damage_per_level", 6d, 0d,
                        Double.MAX_VALUE);

        THERMOHEX_PERCENT_MAX_HEALTH_DOT_BASE = CONFIG_BUILDER.comment("""
                        Thermohex causes burn or freeze DOT to be increased by this much
                        based on victim's max health, in decimal form.
                        """)
                .defineInRange("thermohex_max_health_ratio_base", 0.02d,
                        0d, 1d);

        THERMOHEX_PERCENT_MAX_HEALTH_DOT_PER_LEVEL = CONFIG_BUILDER.comment("""
                        Each level of Thermohex increases the max health ratio by this much
                        in decimal form.
                        """)
                .defineInRange("thermohex_max_health_ratio_per_level", 0.01d,
                        0d, 1d);
        CONFIG_BUILDER.pop();

        CONFIG_BUILDER.push("Black/whitelist");

        PUZZLE_5_EFFECT_BLACKLIST = CONFIG_BUILDER.comment(
                        "List of effects whose duration should not be " + "modified by Puzzle 5.")
                .defineList("puzzle_5_blacklist",
                        List.of("irons_spellbooks:vex_timer",
    ...
    public static final class Cache {
        ...
        public static void onConfigReload(ModConfigEvent.Reloading event) {
            if (event.getConfig().getSpec() == Config.COMMON_CONFIG) {
                puzzle5BlacklistCache = new HashSet<>(
                        Config.PUZZLE_5_EFFECT_BLACKLIST.get());
            }
        }
